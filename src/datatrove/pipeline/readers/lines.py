...
class LinesReader(BaseDiskReader):
    """Read data from Lines files.
        Will read each line as a separate document, 
        where each line is just the text of the document.
                ...
                for li, line in enumerate(f):
                    with self.track_time():
                        try:
                            if not line:
                              continue
                            document = self.get_document_from_dict({self.text_key: line}, filepath, li)
                            if not document:
                                continue
